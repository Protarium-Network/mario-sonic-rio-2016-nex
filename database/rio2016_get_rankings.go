...
// Ranking rows live in dedicated rio2016_* tables. NEX's ranking protocol
// does not scope categories by title, so keeping this title's leaderboards
// in their own tables avoids category-number collisions if this database is
// ever shared with another game.
func Rio2016GetRankingsAndCountByCategoryAndRankingOrderParam(category types.UInt32, rankingOrderParam ranking_types.RankingOrderParam) (types.List[ranking_types.RankingRankData], uint32, error) {
	return Rio2016GetRankings(
...
// Rio 2016 exercises ranking modes range(0), user(4), near(1) and
// friend-range(2). There's no friends system for this title, so friend-range
// degrades to the same query as user-mode (per the protocol's own documented
// fallback: "if the connected user has no friends, functions identically to
// RankingModeUser").
const rankingModeNear = 1
const rankingModeFriendRangeRio2016 = 2

func Rio2016GetRankings(
	...
	if mode == rankingModeFriendRangeRio2016 {
		mode = rankingModeUser
	}
	if mode != rankingModeRange && mode != rankingModeUser && mode != rankingModeNear {
	...
	if mode == rankingModeUser || mode == rankingModeNear {
		offset = 0
	}
	...
	rows, err := Postgres.Query(`
		WITH base AS (
			...
			FROM rio2016_rankings AS r
			LEFT JOIN rio2016_ranking_categories AS rc
				ON rc.category = r.category
			LEFT JOIN rio2016_common_datas AS cd
