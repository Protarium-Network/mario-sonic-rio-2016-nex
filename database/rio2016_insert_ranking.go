...
func Rio2016InsertRankingByPIDAndRankingScoreData(pid types.PID, rankingScoreData ranking_types.RankingScoreData, uniqueID types.UInt64) error {
	...
	result, err := tx.Exec(`
		...
		WHERE
			...
			OR (rio2016_rankings.order_by = 0 AND EXCLUDED.score < rio2016_rankings.score)
			OR (rio2016_rankings.order_by = 1 AND EXCLUDED.score > rio2016_rankings.score)
	`,
		int64(pid),
		int64(uniqueID),
		int64(rankingScoreData.Category),
	...
	globals.Logger.Infof("[RIO2016 RANKING] upload PID=%d category=%d score=%d stored=%t",
